package prosayj.spring.mvc.controller;

import prosayj.spring.mvc.pojo.QueryVo;
import prosayj.spring.mvc.pojo.User;
...
public class DemoController {
     ...
     * 注意：写在这里只会对当前 Controller类生效
     * <pre>
     * @ExceptionHandler(ArithmeticException.class)
     * </pre>
     */
    public void handleException(ArithmeticException exception, HttpServletResponse response) throws Exception {
        // 异常处理逻辑
        String message = exception.getMessage();
        //response.getWriter().write(message);
        System.out.println("DemoController 局部异常处理,异常信息是：" + message);
        throw exception;
    }


    /**
     * <a href="http://localhost:8080/demo/handle01">URL</a>
     */
    ...
    public ModelAndView handle01(@ModelAttribute("name") String name) {
        ...
        //是向请求域中request.setAttribute("date",date);
     ...
     * <p>
     * 如果要在SpringMVC中使用servlet原生对象，比如 HttpServletRequest\HttpServletResponse\HttpSession，直接在Handler方法形参中声明使用即可
     */
    ...
    @RequestMapping("/handle03-2")
    public ModelAndView handle032(@RequestParam("ids") Integer id, Boolean flag) {
        Date date = new Date();
        ModelAndView modelAndView = new ModelAndView();
        modelAndView.addObject("date", date);
        modelAndView.setViewName("success");
